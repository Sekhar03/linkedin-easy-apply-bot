...
async function fillFieldsOnCurrentScreen(page) {
  ...
  const customDropdowns = await page.$(`${prefix}button[aria-haspopup="listbox"], ${prefix}.artdeco-dropdown__trigger`);
...
async function run() {
  ...
  const browser = await chromium.launch({
    ...
    slowMo: 100, // Slows down Playwright operations by 100ms so you can see them
  ...
  try {
    if (config.platform === 'other') {
      ...
      // Wait a few seconds for the page to finish rendering
      await page.waitForTimeout(5000);
    } else {
